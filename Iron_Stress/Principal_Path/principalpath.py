...
def rkm_prefilter(X, boundary_ids, Nf=200, k=5, p=1000, T=0.1, plot_ax=None):
    ...
    if(plot_ax is not None):
        ...
        pyplot.plot(X[filter_mask,0],X[filter_mask,1],'bo',label='data diocane kept')
...
def rkm_MS_evidence(models, s_span, X):
    ...
    for i,s in enumerate(s_span):
        ...
        if(lambd>0):
            ...
            logE = logE + 0.5*d*NC*np.log(lambd+np.finfo(float).eps)
...
def rkm_MS_ksgm(models, s_span, X):
    ...
    for i in range(models.shape[0]):
        ...
        if(np.all(b2>0)):
            ...
            dst2 = a2 - ab*ab / (b2+np.finfo(float).eps)
...
def rkm_MS_gui(models, s_span, X, X_g=None):
    ...
    input('select model with GUI then press [enter] to continue')
